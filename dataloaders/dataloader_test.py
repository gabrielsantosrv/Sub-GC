...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.subgraph_mask = HybridLoader(
            "data/{}_graph_mask_{}_rm_duplicate".format(dataset_name, mask_version), ".npz"
        )
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        tmp_list = self._prefetch_process[
            split
        ].get()  # call one time to get a whole batch instead of fetching one by one instance
        wrapped = tmp_list[-1]
        tmp_list = tmp_list[:-1]
        ...
        for ix in tmp_ix.view(-1).numpy():
            ...
            data["infos"].append(
                {"ix": ix, "id": self.info["images"][ix]["id"], "file_path": self.info["images"][ix]["file_path"]}
            )
    ...
    def __getitem__(self, index):
        ...
        pad_object_fmap = np.full((1, self.obj_num, object_fmap.shape[1]), 0).astype(
            "float32"
        )  # pad with the dummy/empty node
        pad_object_dist = np.concatenate(
            (np.ones((1, self.obj_num, 1)), np.zeros((1, self.obj_num, object_dist.shape[1] - 1))), axis=2
        ).astype("float32")
        ...
        pad_rel_ind = np.full(
            (1, self.rel_num, rel_ind.shape[1]), self.obj_num - 1
        )  # pad the rel_ind with the dummy/empty node index
        pad_pred_dist = np.concatenate(
            (np.ones((1, self.rel_num, 1)), np.zeros((1, self.rel_num, pred_dist.shape[1] - 1))), axis=2
        ).astype("float32")
...
class BlobFetcher:
    ...
    def _get_next_minibatch_inds(self):
        ...
        if self.split == "train" or self.split == "val":  # batch size >= 1, drop last batch
            ...
            if (
                wrapped is False and ri_next + self.batch_size >= max_index
            ):  # the next wrapped will be True, then current batch becomes last batch to be used
                last_batch = True
    ...
    def get(self):
        ...
        # print(">>>>> self.split_loader:", type(self.split_loader))
