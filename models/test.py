...
parser.add_argument("--use_gpn", type=int, default=1, help="1: use GPN module in the captioning model")
parser.add_argument("--embed_dim", type=int, default=300, help="dim of word embeddings")
parser.add_argument("--gcn_dim", type=int, default=1024, help="dim of the node/edge features in GCN")
parser.add_argument(
    "--noun_fuse", type=int, default=1, help="1: fuse the word embedding with visual features for noun nodes"
)
parser.add_argument("--pred_emb_type", type=int, default=1, help="predicate embedding type")
parser.add_argument("--gcn_layers", type=int, default=2, help="the layer number of GCN")
parser.add_argument("--gcn_residual", type=int, default=2, help="2: there is a skip connection every 2 GCN layers")
parser.add_argument("--gcn_bn", type=int, default=0, help="0: not use BN in GCN layers")
parser.add_argument("--sampling_prob", type=float, default=0.0, help="Schedule sampling probability")
parser.add_argument(
    "--obj_name_path", type=str, default="data/object_names_1600-0-20.npy", help="the file path for object names"
)
parser.add_argument(
    "--rel_name_path", type=str, default="data/predicate_names_1600-0-20.npy", help="the file path for predicate names"
)
...
parser.add_argument("--use_MRNN_split", action="store_true", help="use the split of MRNN on COCO Caption dataset")
parser.add_argument(
    "--use_gt_subg",
    action="store_true",
...
parser.add_argument("--obj_num", type=int, default=37, help="the number of detected objects + 1 dummy object")
parser.add_argument(
    "--rel_num", type=int, default=65, help="the number of detected relationships + 1 dummy relationship"
)
...
parser.add_argument("--test_LSTM", type=int, default=1, help="1: generate captions, used during evaluation (testing)")
parser.add_argument("--use_topk_sampling", type=int, default=0, help="1: use topk sampling during decoding each word")
parser.add_argument("--topk_temp", type=float, default=0.6, help="the temperature used in topk sampling")
parser.add_argument("--the_k", type=int, default=3, help="k top candidates are used in sampling")
parser.add_argument("--gpn_nms_thres", type=float, default=0.75, help="the threshold in sub-graph NMS during testing")
parser.add_argument(
    "--gpn_max_subg", type=int, default=1, help="the maximum number of sub-graphs to be kept during testing"
)
...
parser.add_argument(
    "--oracle_num", type=int, default=1, help="how many sentences are used to calculate the top-1 accuracy"
)
# grounding attention triger
parser.add_argument(
    "--return_att", type=int, default=0, help="1: return attention weight for each time step, for grounding evaluation"
)
